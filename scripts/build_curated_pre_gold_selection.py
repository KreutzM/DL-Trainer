...
DEFAULT_TOTAL_TARGETS = {
    "clarification": 12,
    "faq_direct_answer": 12,
    "step_by_step": 12,
    "troubleshooting": 12,
    "uncertainty_escalation": 12,
}

DEFAULT_SPLIT_TASK_TARGETS = {
    ("eval", "clarification"): 6,
    ("eval", "faq_direct_answer"): 6,
    ("eval", "step_by_step"): 6,
    ("eval", "troubleshooting"): 6,
    ("eval", "uncertainty_escalation"): 6,
    ("train", "clarification"): 6,
    ("train", "faq_direct_answer"): 6,
    ("train", "step_by_step"): 6,
    ("train", "troubleshooting"): 6,
    ("train", "uncertainty_escalation"): 6,
}
...
def parse_args() -> Any:
    parser = make_parser("Build a curated larger JAWS-DE OpenRouter pre-gold shadow selection from wave2 scaleup jobs.")
    parser.add_argument("--jobs", default="data/derived/teacher_jobs/JAWS/DE/wave2_scaleup_generation_jobs.jsonl")
    parser.add_argument(
        "--output-job-ids",
        default="data/derived/teacher_jobs/JAWS/DE/shadow_wave_2026_04_04_user_answer_v16_curated_pre_gold_wave_job_ids.txt",
    )
    parser.add_argument(
        "--output-selection",
        default="data/derived/teacher_jobs/JAWS/DE/shadow_wave_2026_04_04_user_answer_v16_curated_pre_gold_wave_selection.json",
    )
    parser.add_argument(
        "--selection-id",
        default="jaws_de_shadow_wave_2026_04_04_user_answer_v16_curated_pre_gold",
    )
    parser.add_argument(
        "--workflow-role",
        default="shadow_wave_user_answer_gpt54_v16_curated_pre_gold",
    )
    parser.add_argument(
        "--target-total-count",
        action="append",
...
def resolved_split_task_targets(args: Any) -> dict[tuple[str, str], int]:
    targets = dict(DEFAULT_SPLIT_TASK_TARGETS)
...
def choose_rows(
    ...
    for split in ("eval", "train"):
        for task_type in TASK_ORDER:
            target_count = split_task_targets[(split, task_type)]
            candidates = [
                row
                for row in rows
    ...
    *,
    ...
    return {
        ...
        "selection_summary": {
            ...
            "selection_strategy": (
                "Deterministically selects a larger curated wave from wave2_scaleup jobs without reusing source_chunk_ids. "
                "Rows are chosen by split/task target, preferring lower already-selected document counts, then higher "
                "fixture quality_score, then stable doc_id/job_id ordering."
            ),
        },
